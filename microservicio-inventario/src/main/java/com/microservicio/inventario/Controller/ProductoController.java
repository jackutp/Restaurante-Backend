...
public class ProductoController {
    ...
    public ResponseEntity<?> details(@PathVariable Long id) {
        Optional<ProductoDTO> opt = service.findById(id);
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> update(@PathVariable Long id, @RequestBody CrearProductoDTO producto) {
        Optional<ProductoDTO> opt = service.update(id, producto);
        if (opt.isPresent()) return ResponseEntity.ok(opt.get());
        return ResponseEntity.notFound().build();
    }
